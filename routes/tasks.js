module.exports = app => {
	...
	.all(app.auth.authenticate())
	     ...
	     */
		.get((req, res) => {
			Tasks.findOne({where: {id: req.params, user_id: req.user.id}})
				.then(result => {
					if (result) {
						res.json(result);
					}else{
	     ...
	     */
		.put((req, res) => {
			Tasks.update(req.body, {where: {id: req.params, user_id: req.user.id}})
	     ...
	     */
		.delete((req, res) => {
			Tasks.destroy({where: {id: req.params, user_id: req.user.id}})
				.then(result => res.sendStatus(204))
				.catch(error => {
					res.status(412).json({msg: error.message});
				});
